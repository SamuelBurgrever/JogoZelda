...
namespace Plugins.SQLite
{
    public class DatabaseService : IDatabaseService
    {
        ...
        public async Task Init()
        {
            if (_db is not null)
                return;
            var dbPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "zelda.db");
        ...
        {
            await Init();
            return await _db.Table<PlayerData>().OrderByDescending(x => x.Id).FirstOrDefaultAsync();
        }
